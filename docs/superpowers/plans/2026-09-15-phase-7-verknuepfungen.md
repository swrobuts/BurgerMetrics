...
**Goal:** Startseite, README und Dokumentation des BM-Repos verweisen auf Rezensionen, Notebooks, Dash und das BM-Lab; die Seiten-Importe tragen den Versionsparameter; ein Bildschirmfoto des Labs liegt für das Deck bereit; letzter PR der Reihe.

**Architecture:** Nur Verknüpfungen und Texte — keine neue Funktion. Vier Tasks: Startseite (`web/index.html` mit vierter Kachel, Tastenkürzel, Satz zu Notebooks/Dash) und Versionsparameter der Modul-Importe; Dokumentation (`README.md`, `db/README.md`, `docs/02`, `05`, `07`, `08`); Kleinigkeiten aus dem Endreview von Phase 6 (`dash/README.md`, `.env.example`, Spec §9.1) plus Lab-Bildschirmfoto für das Deck; Abnahme mit Spec-Absatz und PR.

**Tech Stack:** HTML/CSS (Startseite), Markdown, Playwright-MCP für die Sichtprüfung, `node --test web/tests/*.test.mjs` und `python3 web/tests/skripte_pruefen.py` als Regressionsprüfung, `slides/screenshots.mjs` (Playwright, git-ignoriert) für das Bildschirmfoto.
...
- **Deck:** Roberts überarbeitetes Deck liegt unter `Vorlesungen/Übergreifend/Fallstudien/BurgerMetrics/BurgerMetrics_Analyse.pptx` (47 Folien, ohne das Kapitel „Abschluss“ des Bauskripts). Phase 7 baut das Deck **nicht** neu und ändert es nicht; sie legt nur das Bildschirmfoto `slides/bilder/16_lab_uebersicht.png` an (Robert entscheidet, ob und wo es ins Deck kommt). `slides/` ist git-ignoriert.
...
### Task 3: Kleinigkeiten aus dem Endreview und Lab-Bildschirmfoto
...
- Modify (git-ignoriert): `slides/screenshots.mjs` (Liste `AUFNAHMEN`), `slides/README.md` (Liste der Bildschirmfotos)
- Create (git-ignoriert): `slides/bilder/16_lab_uebersicht.png`
...
- [ ] **Step 4: Bildschirmfoto** — in `slides/screenshots.mjs` einen Eintrag `{ name: "16_lab_uebersicht", seite: "lab/index.html", breite: 1180, hoehe: 900, warte: 1200, wahl: ".labs-grid" }` (Selektor gegen `web/lab/index.html` prüfen; ggf. `hoeheAnteil` wie bei Eintrag 12) ergänzen und mit laufendem Server `BASIS=http://localhost:8731 node slides/screenshots.mjs 16_lab_uebersicht` erzeugen; Bild ansehen (acht Karten sichtbar, keine Ladeanzeige). In `slides/README.md` die Liste der Bildschirmfotos um `16_lab_uebersicht` ergänzen und die Zeile zum Deck 3 um den Hinweis, dass Roberts überarbeitete Fassung unter `Vorlesungen/Übergreifend/Fallstudien/BurgerMetrics/BurgerMetrics_Analyse.pptx` liegt (47 Folien, ohne Kapitel Abschluss) und das Bauskript sie nicht überschreibt.
- [ ] **Step 5: Commit** (nur die versionierten Dateien): `docs: dash/README, .env.example, Spec §9.1 (Endreview Phase 6)`
...
- [ ] **Step 2: Spec-Absatz** — „**Stand <Datum> (Phase 7):** Verknüpfungen abgenommen — …“ mit: Kachel und Taste 4, Satz zu Notebooks/Dash, Versionsparameter `?v=20260915` auf allen vier Seiten, README-Abschnitte und Tabelle, `db/README.md` (503), docs 02/05/07/08 (fact_reviews im Diagramm, Shop-Feature, Abschnitt 7.7, E10), `dash/README.md`, `.env.example`, §9.1-Baum, Bildschirmfoto `16_lab_uebersicht` (Deck unverändert: Roberts Fassung unter Übergreifend/Fallstudien, Entscheidung über die Folie bei Robert); damit ist die Übergabe je Phase aus §11 vollständig erfüllt.
