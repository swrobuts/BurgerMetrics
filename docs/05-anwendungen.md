...
**Bewerten.** Auf der Produktkarte öffnet der Knopf „Bewerten“ ein Modal mit Sternewahl, Text und Filiale; Speichern ruft `wawi.rezension_anlegen()` per PostgREST-RPC unter der Rolle `anon` auf. Die Prüfungen — Sterne 1 bis 5, Text 5 bis 500 Zeichen, höchstens 20 Rezensionen je Sitzung in zehn Minuten und 600 je Stunde insgesamt — liegen in der Funktion; der Browser prüft nur vorab. Greift die Bremse, antwortet PostgREST mit HTTP 503 und der Meldung der Datenbank. Öffentlich sichtbar sind nur Aggregate (`v_rezension_produkt`); Besuchertext erscheint auf keiner Seite. Die Logik steht in `web/js/rezensionen.js`, geprüft mit `node --test web/tests/*.test.mjs`. Die Übernahme ins Auswertungsmodell ist wie bei Bestellungen ein bewusster Aufruf des Betreibers (`uebernahme_aus_wawi()`); `uebungsrezensionen_loeschen()` setzt zurück.
